...
//
// `reporters/*` get per-file outputs so the lazy `await import()` calls in
// `reporters/registry.ts` resolve at runtime; they're externalised from the
// runner/common bundles to keep startup cost lazy.
steps.push(new EsbuildStep({
  entryPoints: [
...
//
// Reporter source files are externalised so the lazy `await import()` calls
// in `reporters/registry.ts` resolve to the per-file `.js` outputs at runtime
// instead of being bundled inline. This keeps the common bundle slim and
// makes reporter-class parse/eval cost lazy (per user's `reporter:` config)
...
steps.push(new EsbuildStep({
  ...
  external: [
    ...
    'better-sqlite3',
    // Reporter sources — see lazy-load note above.
    '../reporters/ai.js',
    '../reporters/blob.js',
    '../reporters/catalog.js',
    '../reporters/csv.js',
    '../reporters/dot.js',
    '../reporters/empty.js',
    '../reporters/github.js',
    ...
    '../reporters/line.js',
    '../reporters/list.js',
    '../reporters/listModeReporter.js',
    '../reporters/newRelic.js',
    '../reporters/xray.js',
  ],
  plugins: [dynamicImportToRequirePlugin],
}, [filePath('packages/playwright/src')]));
...
// Reporter sources externalised — see common/index.js note. The runner only
// loads the reporters the user's config requests, via lazy `await import()`.
steps.push(new EsbuildStep({
  bundle: true,
...
// `child_process.fork(path)` chokes on `.js` entries in a `"type":"module"`
// workspace under Node 25 (ENOENT on file:// resolution). The runner's
// `processHost.ts` rewrites `entry.js` → `entry.cjs` at fork time; we emit
// a tiny CJS shim next to each forked entry that just `require()`s the
// sibling ESM module (Node 22.12+ supports sync require of ESM).
function emitForkCjsShims() {
  const targets = [
  ...
  for (const jsTarget of targets) {
    const jsPath = filePath(jsTarget);
    if (!fs.existsSync(jsPath))
      continue;
    const cjsPath = jsPath.replace(/\.js$/, '.cjs');
    const baseName = path.basename(jsPath);
    const shim = `// Auto-generated by utils/build/build.js. Forwards child_process.fork's
// entry to the ESM \`.js\` sibling. Node 25 fails on .js fork entries inside
// a "type":"module" workspace; the .cjs lives at a path that bypasses that.
module.exports = require('./${baseName}');
`;
    fs.writeFileSync(cjsPath, shim);
  }
}
steps.push(new CustomCallbackStep(emitForkCjsShims));
