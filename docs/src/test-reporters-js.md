...
Catalog reporter is a list-style alternative that buckets tests by a logical "area" (team, product surface) and prints per-bucket percentile durations along with the per-test pass/fail/flaky line. Useful when a test run spans multiple product areas and you want both a live tick stream and an end-of-run summary.

```bash
npx playwright test --reporter=catalog
```
...
```bash
  ✓  signup › creates account  [tests/auth/signup.spec.ts]  82ms
  ✗  checkout › invalid card   [tests/billing/checkout.spec.ts]  431ms
     expect(received).toBe(expected)

Catalog summary
═══════════════
  auth                 12P 0F 0Fk 0S        p50=78ms  p90=145ms  p95=220ms
  billing              4P 1F 0Fk 0S         p50=210ms  p90=431ms  p95=431ms
  core                 28P 0F 1Fk 0S        p50=92ms  p90=180ms  p95=246ms

status: failed
```

Catalog reporter supports the following configuration options:

| Reporter Config Option | Description | Default |
|---|---|---|
| `productAreaResolver` | `(testFile: string) => { area?: string }` — maps a test file path to a bucket name. | All tests bucketed under `default`. |
| `quiet` | Skip the per-test line, only print the summary. | `false` |
...
CSV reporter writes one row per test (RFC-4180-compliant escaping). Useful when test results need to flow into a spreadsheet, BigQuery, or any tabular consumer.

```bash
npx playwright test --reporter=csv
```
...
export default defineConfig({
  reporter: [['csv', { outputFile: 'reports/results.csv' }]],
});
```

Columns: `file`, `project`, `title`, `fullTitle`, `status`, `durationMs`, `retries`, `ticketId`, `error`. Pass a `ticketPattern` regex to extract a ticket id from each test title — the captured group is written into the `ticketId` column.
...
export default defineConfig({
  reporter: [['csv', {
    ...
    ticketPattern: '\\[(QE-\\d+)\\]', // pulls "QE-123" out of "[QE-123] login works"
  }]],
});
```

CSV reporter supports the following configuration options and environment variables:

| Environment Variable Name | Reporter Config Option | Description | Default |
|---|---|---|---|
| `PLAYWRIGHT_CSV_OUTPUT_DIR` | `outputDir` | Directory to save the report to. | `playwright-report` |
| `PLAYWRIGHT_CSV_OUTPUT_FILE` | `outputFile` | Full path to the output file. | `<outputDir>/results.csv` |
|  | `ticketPattern` | Regex applied to the test title to extract a ticket id. | No ticket extraction. |
|  | `noHeader` | Skip the header row. Useful when appending to an existing file. | `false` |

### AI reporter

AI reporter produces an LLM-consumable view of failed tests — per-failure markdown briefings, a `failures.jsonl` for batch consumption, and an `index.md` to read first. Each briefing ends with a "How to investigate" block teaching the LLM to use Playwright's CLI (`show-trace`, `show-report`, `codegen`) for deeper inspection.

```bash
npx playwright test --reporter=ai
```
...
export default defineConfig({
  reporter: [['ai', {
    outputDir: 'playwright-report/ai',
    prompt: 'You are the QA triage bot. Always suggest a Jira label.',
  }]],
});
```

Each failure briefing contains: test identity (file/line/project/tags), error message + stack, source frame, categorised console/stderr output, attachment inventory (trace, video, screenshots), CI metadata, and the CLI investigation block. The `prompt` option prepends a project-specific triage prompt above the briefing — accepts an inline string or a path to a markdown file.

AI reporter supports the following configuration options and environment variables:

| Environment Variable Name | Reporter Config Option | Description | Default |
|---|---|---|---|
| `PLAYWRIGHT_AI_OUTPUT_DIR` | `outputDir` | Directory to save briefings to. | `playwright-report/ai` |
|  | `prompt` | Inline prompt string, or path to a markdown file. | No prompt prepended. |
|  | `markdown` | Emit per-failure markdown briefings + index. | `true` |
|  | `jsonl` | Emit `failures.jsonl`. | `true` |

### Slack reporter

Slack reporter POSTs failure summaries (or per-failure messages) to a Slack incoming webhook. Auto-detects CI metadata (provider, branch, commit, run URL, PR link) and surfaces it in the message.

```bash
SLACK_WEBHOOK_URL=https://hooks.slack.com/services/... npx playwright test --reporter=slack
```
...
export default defineConfig({
  reporter: [['slack', {
    ...
    mode: 'summary',
    branchFilter: 'main',
    mention: ['U123', 'U456'], // user IDs to @mention on failure
  }]],
});
```

Slack reporter supports the following configuration options:

| Reporter Config Option | Description | Default |
|---|---|---|
| `webhookUrl` | Slack incoming webhook URL. Reporter is a no-op without one (unless `dryRun: true`). | — |
| `mode` | `'per-test'` (one message per failure), `'summary'` (one message at end of run), or `'both'`. | `'summary'` |
| `branchFilter` | If set, only fire when the CI branch matches. | Fire on all branches. |
...
export default defineConfig({
  reporter: [['intellum-social', {
    webhookUrl: process.env.SOCIAL_WEBHOOK_URL,
    mode: 'summary',
  }]],
});
```
...
Jira reporter auto-creates bug tickets for failing tests, dedupes via JQL search against the project, and links each ticket back to the CI run. Opt-in by default — set `enabled: true` to activate.
...
export default defineConfig({
  reporter: [['jira', {
    enabled: true,
    baseUrl: process.env.JIRA_INSTANCE_URL,
    projectKey: 'QE',
    auth: {
      email: process.env.JIRA_EMAIL!,
      token: process.env.JIRA_TOKEN!,
    },
    labels: ['nightly', 'auto-triage'],
    githubBaseUrl: 'https://github.com/org/repo',
  }]],
});
```

Jira reporter supports the following configuration options:

| Reporter Config Option | Description | Default |
|---|---|---|
| `enabled` | Master switch. **Must be `true` for tickets to be created.** | `false` |
| `baseUrl` | Jira instance URL (e.g. `https://acme.atlassian.net`). | — |
| `projectKey` | Project key tickets are created under. | — |
| `auth` | `{ email, token }` — Jira API token credentials. | — |
| `issueType` | Issue type name. | `'Bug'` |
| `labels` | Labels added to every ticket. `'playwright'` is always appended. | `[]` |
| `productAreaMapper` | `(testFile: string) => { area?, component? }` — routes tickets via component / area label. | — |
| `githubBaseUrl` | Base URL for source links in the ticket description. | — |
| `dryRun` | Print issue payloads to stderr instead of POSTing. | `false` |

### Xray reporter

Xray reporter posts test execution results to Xray (Jira test management). Extracts test keys from test titles via a configurable regex.
...
Tests are matched against the configured pattern (default `\[([A-Z]+-\d+)\]`, e.g. `[QE-1]`). Tests without a matching key in their title are silently skipped — only tagged tests reach Xray.

Xray reporter supports the following configuration options:

| Reporter Config Option | Description | Default |
|---|---|---|
| `baseUrl` | Xray cloud or server URL. | — |
| `auth` | `{ clientId, clientSecret }` or `{ token }`. | — |
| `testPlan` | Xray test plan key to attach the execution to. | — |
| `testExecutionKey` | Specific test execution to update (instead of creating a new one). | New execution per run. |
| `testKeyPattern` | Regex used to extract a test key from each title. Capture group 1 is used; falls back to the whole match. | `\[([A-Z]+-\d+)\]` |
| `dryRun` | Print payloads to stderr instead of POSTing. | `false` |

### New Relic reporter

New Relic reporter sends one custom event per test to NR Insights. Useful for dashboarding test duration / pass-rate / flakiness over time.
...
export default defineConfig({
  reporter: [['new-relic', {
    ingestKey: process.env.NR_INGEST_KEY,
    accountId: process.env.NR_ACCOUNT_ID,
    region: 'US',
    eventType: 'PlaywrightTestRun',
    extraAttributes: { service: 'web-app', env: 'staging' },
  }]],
});
```

Each event carries: test title, file, project, status, duration, retry, error summary, plus CI metadata (provider, branch, sha, run URL, job name).

New Relic reporter supports the following configuration options:

| Reporter Config Option | Description | Default |
|---|---|---|
| `ingestKey` | NR Insights ingest key. Reporter is a no-op without one. | — |
| `accountId` | NR account id. | — |
| `region` | `'US'` or `'EU'` — selects the insights collector endpoint. | `'US'` |
| `eventType` | Custom event type name in NR. | `'PlaywrightTestRun'` |
| `flatten` | Spread error fields onto the event (`errorMessage`, `errorStack`). When `false`, nest under `errors`. | `true` |
| `extraAttributes` | Static attributes mixed into every event. | — |
| `endpoint` | Override the collector URL (for testing). | Auto from `region`. |
| `dryRun` | Print payloads to stderr instead of POSTing. | `false` |
