...
import { shouldProxyLoopback, CRBrowser } from './crBrowser.js';
import { ConnectionEvents, CRConnection, kBrowserCloseMessageId } from './crConnection.js';
...
import type { BrowserOptions, BrowserProcess } from '../browser.js';
import type { SdkObject } from '../instrumentation.js';
import type { Progress } from '../progress.js';
import type { ConnectionTransport, ProtocolRequest } from '../transport.js';
  ...
  override async connectToWorker(progress: Progress, endpoint: string) {
    ...
    try {
      ...
      session.on('Runtime.executionContextCreated', event => {
        const isDefault = event.context.auxData?.isDefault as unknown as boolean | undefined;
        if (isDefault === false) {
